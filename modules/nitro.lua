local function draw(w, ctx)
  ...
  -- Block 1: ENGINE TEMP
  text(cx1 + 123, 260, (ctx.T and ctx.T("engine_temp")) or "ENGINE TEMP", CENTER + f_sml, C.dim)
  ...
  text(cx1 + 123, 362, string.format((ctx.T and ctx.T("sub_max_t")) or "max %.0f°C", max_temp), CENTER + f_sml, C.dim)

  -- Block 2: RX PACK
  text(cx2 + 123, 260, (ctx.T and ctx.T("rx_pack_title")) or "RX PACK", CENTER + f_sml, C.dim)
  ...
  text(cx2 + 123, 362, string.format((ctx.T and ctx.T("sub_min_v_rx")) or "min %.1fV", min_vbec), CENTER + f_sml, C.dim)
end
