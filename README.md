...
* **穩定性修復**：徹底根治長時長飛行後單包電池彈窗 CPU Limit 報錯。
  * **走勢圖步進降採樣**：Battery Profile 走勢圖限制最多 35 點繪製，避免 200 點累積造成高頻重繪。
  * **Power Stats 降採樣**：動力曲線繪製上限調整為 35 點，降低單幀線條計算。
  * **彈窗與硬體開關解耦**：以 `viewing_bat_idx` 瀏覽電池，不再誤觸發硬體電池切換與記憶卡重讀。
  * **彈窗背景節流**：彈窗開啟時略過被遮擋的模型圖片縮放運算。
  * **旋翼動力學與 6POS 優化**：修正 Rotor Dynamics 拉力參數，並預先配置 6POS 開關名稱以降低 GC。
* **功能新增**：新增光感進入／退出值與強光主題選擇，提供可調回差門檻。
* **相容性與 SD 效能**：新選項以尾端追加方式保留既有設定索引；曲線資料改為單次原子寫入，減少降落時 SD I/O 壓力。
